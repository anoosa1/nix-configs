...
{
  flake.nixosModules.minecraft = { pkgs, config, ... }:
    let
      ...
      gravitas2-files = pkgs.runCommand "atm-gravitas2-0.9.4" {
        nativeBuildInputs = [ pkgs.unzip ];
      } ''
        mkdir -p $out
        unzip ${../ATMGravitas2-server-0.9.4.zip} -d $out/
      '';

      forgeServer = self.packages.${pkgs.stdenv.hostPlatform.system}.forge-server or
        (throw "forge-server package not found. Make sure modules/pkgs/forge-server.nix is tracked by git.");
    in
    {
    imports = [
    ...
    services = {
      minecraft-servers = {
        ...
        servers = {
          # --- Existing SkyFactory (Fabric) server ---
          #hardcore = {
          #  enable = true;
          #  autoStart = true;
          #  openFirewall = true;
          ...
          # --- ATM Gravitas 2 (Forge 1.20.1) server ---
          # Disabled: running manually instead
          gravitas2 = {
            enable = false;
            autoStart = false;
            openFirewall = true;

            # Heavy pack: GregTech + TerraFirmaCraft + 300+ mods
            jvmOpts = "-Xms8192M -Xmx16384M -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20 -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1";

            package = forgeServer;

            serverProperties = {
              difficulty = 3;
              motd = "\\u00a7b\\u00a7lATM Gravitas 2 \\u00a7r\\u00a78| \\u00a7aGregTech+TFC";
              server-port = 25566;
              allow-flight = true;         # REQUIRED for modded
              spawn-protection = 0;
              max-tick-time = 180000;      # modded worldgen needs longer ticks
            ...
            # Symlink Forge libraries and mods into the server data directory.
            # The launch script expects libraries/ and mods/ relative to its cwd.
            symlinks = {
              "libraries" = "${forgeServer}/libraries";
              "mods" = "${gravitas2-files}/mods";
            };

            # Copy config + extras from the server pack
            files = {
              "config" = "${gravitas2-files}/config";
              "defaultconfigs" = "${gravitas2-files}/defaultconfigs";
              "kubejs" = "${gravitas2-files}/kubejs";
              "packmenu" = "${gravitas2-files}/packmenu";
            };
          };
        };
      };
